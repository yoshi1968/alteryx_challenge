...
このリポジトリは、[プロジェクトの目的：例 毎月の売上集計およびレポート作成] を自動化するための Alteryx ワークフローを管理します。

---

## 前提条件

* **Alteryx Designer バージョン:** `2023.1` 以上推奨
* **必要なツール / Add-on:**
* **データ接続 (DCM):**
* DB接続名は `DB_CONNECTION_PROD` で統一してください。
...
* **一時ファイル:** `.bak` ファイルや `_Externals` フォルダが混入していないか確認してください。
